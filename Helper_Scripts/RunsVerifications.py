...
with open('/home/adeye/Experiment_Results/ExperimentA.csv') as csvfile:
...
for trigger_ditance in TRIGGER_DISTANCES:
    for speed in EGO_SPEEDS:
        for intensity in RAIN_INTENSITIES:
            for reflectivity in REFLECTIONS:
                index += 1
                if parameter_set[trigger_ditance][speed][intensity][reflectivity][0] != 1 and index >= 1 and index < 3077:
