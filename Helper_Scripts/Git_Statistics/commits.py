...
def GenerateDateStringList():
    ...
    for month in range (10,13):
        dates_strings.append('2018-'+str(month))
    for year in range (2019,last_year):
        ...
        for month in range (1,10):
            dates_strings.append(str(year)+'-0'+str(month))
        for month in range (10,13):
            dates_strings.append(str(year)+'-'+str(month))
    # We still have to separate the cases where the month number is 1 character and the cases where it is 2 characters
    if last_date[5] == '0':
        for month in range (1,last_month+1):
            dates_strings.append(str(last_year)+'-0'+str(month))
    else:
        for month in range (1,10):
            dates_strings.append(str(year)+'-0'+str(month))
...
for index in range (len(repositories_names)) :
    ...
    for i in range(len(dates_strings)):
        for j in range (len(x_legend_strings)):
            if x_legend_strings[j]==dates_strings[i]:
...
plt.xticks(dates,dates_strings,rotation=60)
plt.plot(dates,number_of_commits)
...
plt.clf()
plt.xticks(dates,dates_strings,rotation=60)
plt.hist(commits_indices,bins=range(len(dates_strings)+1))
