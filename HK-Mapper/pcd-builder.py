...
"""
    ...
    Description: Starts ndt mapping of a pre-recorded rosbag and create a PCD.
...
def create_parser(): 
    ...
    parser = argparse.ArgumentParser(description='Starts ndt mapping of a pre-recorded rosbag and create a PCD.')
    parser.add_argument('-r', '--rosbag', type=bool, default=True, help="Run with rosbag <True|False>")
    parser.add_argument("--rosbag-file", type=str, help="Path to rosbag file")
...
def generate_PCD_path(args): 
    """Generate a timestamp for the PCD"""

    output_filename = os.path.basename(os.path.abspath(args.rosbag_file)).split(".bag")[0] + ".pcd"
...
class Roslaunch(): 
    ...
    def transform_imu(self):
        """Transforms origo to take in roll, pitch, yaw"""
        rospy.init_node('test')
        msg = rospy.wait_for_message("/imu_raw", Imu)               #Creating a new subscription to the topic, receiving one message, then unsubscribing.
    ...
    def play_rosbag(self):
        ...
        rosbag_path = self.args.rosbag_file
    ...
    def start_ndt(self):
        ...
        settings = "{header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''}, resolution: 1.0, step_size: 0.1, trans_epsilon: 0.01, max_iterations: 50, leaf_size: 1.15, min_scan_range: 1.0, max_scan_range: 75.0, min_add_scan_shift: 2.0}"

        cmd = ["rostopic", "pub", "/config/ndt_mapping", "autoware_config_msgs/ConfigNDTMapping", settings]
        subprocess.Popen(cmd)

        rospy.set_param("/ndt_mapping/imu_topic", "/imu_raw")
    ...
    def start(self): 
        ...
        try: 
            while True:
                ...
                rospy.wait_for_message("/ndt_map", PointCloud2, timeout=30)
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(args.rosbag_file):                #Error handling if file does not exist 
        raise OSError(2, 'No such file or directory', args.rosbag_file)
