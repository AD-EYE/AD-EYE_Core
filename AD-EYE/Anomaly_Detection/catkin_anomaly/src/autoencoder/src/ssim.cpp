...
double calcSSIM(const cv::Mat& src1,const cv::Mat& src2, int channel, int method,const cv::Mat& mask, const double K1, const double K2,	const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma)
{
	cv::Mat ssim_map (src1.size(),CV_8U);
    
    IplImage isrc1 = IplImage(src1),isrc2 = IplImage(src2), issim_map = IplImage(ssim_map);
    
	if(mask.empty())
	{
		xcvCalcSSIM(&isrc1,&isrc2,channel,method,NULL,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
		return xcvCalcSSIM(&isrc1,&isrc2,channel,method,NULL,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
	}
	else {
        IplImage imask = IplImage(mask);
        
		return xcvCalcSSIM(&isrc1,&isrc2,channel,method,&imask,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
    }
}
...
double calcSSIMBB(cv::Mat& src1, cv::Mat& src2, int channel, int method, int boundx, int boundy, const double K1, const double K2, const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma, cv::Mat& ssim_map)
{
    ...
    IplImage isrc1 = IplImage(src1),isrc2 = IplImage(src2), issim_map = IplImage(ssim_map);
	return xcvCalcSSIMBB(&isrc1,&isrc2,channel,method,boundx,boundy,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
}
...
double calcDSSIM(cv::Mat& src1, cv::Mat& src2, int channel, int method, cv::Mat& mask, const double K1, const double K2,	const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma, cv::Mat& ssim_map)
{
	...
	{
		xcvCalcSSIM(&isrc1,&isrc2,channel,method,NULL,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
		return xcvCalcDSSIM(&isrc1,&isrc2,channel,method,NULL,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
	}
	else {
        IplImage imask = IplImage(mask);
        
		return xcvCalcDSSIM(&isrc1,&isrc2,channel,method,&imask,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
    }
}
...
double calcDSSIMBB(cv::Mat& src1, cv::Mat& src2, int channel, int method, int boundx, int boundy, const double K1, const double K2, const int L, const int downsamplewidth, const int gaussian_window, const double gaussian_sigma, cv::Mat& ssim_map)
{
	...
	return xcvCalcDSSIMBB(&isrc1,&isrc2,channel,method,boundx,boundy,K1,K2,L,downsamplewidth,gaussian_window,gaussian_sigma, &issim_map);
}
