...
for j =1:length(models.worldmodel.object) %main for loop
            ...
            if(fieldexists(Struct_OpenSCENARIO, "Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private"))  %if Private exists               
                        ...
                        if(isfield(convertCharsToStrings(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}),'PrivateAction') == 1 )  %if Action exists                        
                            for p = 1:length(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}.PrivateAction) %for each Action
                                ...
                                if(isfield(convertCharsToStrings(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}.PrivateAction{1,p}),'LongitudinalAction') == 1 )  %if Speed exists
                                    ...
                                    if(fieldexists(Struct_OpenSCENARIO, strcat("Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, ", num2str(k),"}.PrivateAction{1,", num2str(p),"}.LongitudinalAction.SpeedAction.SpeedActionDynamics.Attributes.dynamicsShape")))
                                    ...
                                    if(fieldexists(Struct_OpenSCENARIO,strcat("Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, ", num2str(k),"}.PrivateAction{1,", num2str(p),"}.LongitudinalAction.SpeedAction.SpeedActionTarget.AbsoluteTargetSpeed")))
