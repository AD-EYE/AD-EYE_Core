                        ...
                        if(isfield(convertCharsToStrings(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}),'PrivateAction') == 1 )  %if Action exists                        
                            for p = 1:length(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}.PrivateAction) %for each Action
                                ...
                                if(isfield(convertCharsToStrings(Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, k}.PrivateAction{1,p}),'LongitudinalAction') == 1 )  %if Speed exists
                                    ...
                                    if(field_exists(Struct_OpenSCENARIO, strcat("Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, ", num2str(k),"}.PrivateAction{1,", num2str(p),"}.LongitudinalAction.SpeedAction.SpeedActionDynamics.Attributes.dynamicsShape")))
                                    ...
                                    if(field_exists(Struct_OpenSCENARIO,strcat("Struct_OpenSCENARIO.OpenSCENARIO.Storyboard.Init.Actions.Private{1, ", num2str(k),"}.PrivateAction{1,", num2str(p),"}.LongitudinalAction.SpeedAction.SpeedActionTarget.AbsoluteTargetSpeed")))
