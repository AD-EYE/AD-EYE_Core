...
ScenarioExpNameArray = listOfNames2;
...
function setSpeedEgo(start_val, step, end_val)
    ...
    %duplicateConfigs(nb_steps);
    for i = 1:nb_steps
        table = readtable(AutowareConfigArray(i+old_nb_experiments-1));
        row = find(strcmp('maxVelocity',table{:,3}));
        table{row,7} = {num2str(start_val+old_nb_experiments)};
        insertBefore(AutowareConfigArray(i),'.xlsx',strcat('_',num2str(i)));
        writetable(table,insertBefore(AutowareConfigArray(i+old_nb_experiments-1),'.xlsx',strcat('_',num2str(i))));
        AutowareConfigArray(i+old_nb_experiments-1)=insertBefore(AutowareConfigArray(i+old_nb_experiments-1),'.xlsx',strcat('_',num2str(i)));
    end
end
