% DMAPI_NCAP_AEBS_Test_T1_CCR_m.m Function to run Euro NCAP test protocol
% for AEB systems: CCRm scenario
...
%% Inputs
speed = (30:5:80).';                % Speed, km/h (Sec. 8.2.3)
overlap = [-50 -75 100 75 50].';    % Lateral overlap, % (Fig. 3)

%% Data Model API (DMAPI)
% Read Prescan data model
model = prescan.experiment.readDataModels('NCAP_AEBS_Test_T1_CCR_m.pb');
...
%% Simulink model: NCAP_AEBS_Test_T1_CCR_m_cs
...
for c = 1:length(overlap)
    ...
    for v = 1:length(speed)
        ...
        if vali == 1
            disp(['Running AEB CCR_m test ' num2str((c-1)*Num_v+v) '/' num2str(Num_c*Num_v) ': Overlap ' num2str(overlap(c)) '%, Velocity ' num2str(speed(v)) ' km/h']);
...
disp('All AEB CCR_m tests completed')