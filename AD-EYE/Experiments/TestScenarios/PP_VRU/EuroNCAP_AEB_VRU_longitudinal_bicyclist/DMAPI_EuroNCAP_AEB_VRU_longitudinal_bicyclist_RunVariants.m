% DMAPI_NCAP_AEB_VRU_longitudinal_bicyclist_RunVariants.m % Script to run Euro NCAP VRU test protocol
% for AEB systems: Car-to-Bicyclist Longitudinal Adult% (CBLA-25, CBLA-50)
...
% Car-to-Bicyclist Longitudinal Adult 25%: CBLA-25
...
cyclistSpeed = 15;                  % Speed, km/h

TTC = 4.5;                          % T0 = TTC 4s defined in section 7.4.2
Smin = 28;                          % VRU 28m from Impact Point -- defined in section 7.4.2

%% Data Model API (DMAPI)
% Read Prescan data model
model = prescan.experiment.readDataModels('EuroNCAP_AEB_VRU_longitudinal_bicyclist.pb');
...
index_Target = prescan.worldmodel.objectsFindByName(model.worldmodel, 'VRU'); 
...
length_VRU = model.worldmodel.object{2, 1}.boundingSize.x;               % Length of VRU, m

% Widths of vehicles
width_VUT = model.worldmodel.object{index_VUT}.boundingSize.y;           % Width of VUT, m
width_VRU = model.worldmodel.object{2, 1}.boundingSize.y;           % Width of Target, m

% Front location VUT x direction, m
front_VUT_x = x0_VUT - cobbOffset_VUT_x + length_VUT;

%% Simulink model: EuroNCAP_AEB_VRU_longitudinal_bicyclist_cs
...
for c = 1:length(overlap)
    ...
    for v = 1:length(speed)
        ...
        deltaV = HostVelocity - (cyclistSpeed/3.6);
        S_TTC = deltaV*TTC;
        ...
        model.scenariomodel.catalogs.maneuverCatalog.maneuver{3, 1}.event{1, 1}.startConditions{1, 1}.condition{1,1}.byEntity.traveledDistance = loc_VRU;
        
        % Validation, writing and running of new model
        vali = prescan.experiment.validate(model);                  % Validate model
        if vali == 1
            disp(['Running AEB VRU longitudinal bicyclist test ' num2str((c-1)*Num_v+v) '/' num2str(Num_c*Num_v) ': Overlap ' num2str(overlap(c)) '%, Velocity ' num2str(speed(v)) ' km/h']);
...
disp('All AEB VRU longitudinal bicyclist tests completed')