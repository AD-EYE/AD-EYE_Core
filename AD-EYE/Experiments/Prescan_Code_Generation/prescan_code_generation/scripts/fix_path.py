...
def format_make_ps_linux_file(experiment_name):
    fix_start_dir_path()
    remove_windows_library_include()
    add_rosbridge_include_path(experiment_name)
    add_rosbridge_build_rule(experiment_name)
    fix_rt_logging_path()
...
def add_rosbridge_include_path(experiment_name):
    EXPERIMENT_INCLUDE_RULE = (
        '-I$(START_DIR)/{experiment_name}_cs_ps_rtw'
...
def add_rosbridge_build_rule(experiment_name):
    EXPERIMENT_BUILD_RULE_FIRST_LINE = (
        '%.o : $(START_DIR)/{experiment_name}_cs_ps_rtw/%.c'
        .format(experiment_name=experiment_name)
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Fixes the make_ps_linux.mk file for PreScan generated code in Linux'
    )
    parser.add_argument('--experiment-name', default=DEFAULT_EXPERIMENT_NAME)
    experiment_name = parser.parse_args().experiment_name

    MAKE_PS_LINUX_FILENAME = (
        '{experiment_name}_cs_ps_rtw/make_ps_linux.mk'
        .format(experiment_name=experiment_name)
    )
    ...
    format_make_ps_linux_file(experiment_name)
