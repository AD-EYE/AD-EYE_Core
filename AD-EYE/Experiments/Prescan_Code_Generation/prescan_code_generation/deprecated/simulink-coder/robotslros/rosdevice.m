classdef rosdevice < ...
        robotics.codertarget.internal.LinuxSystemInterface & ...
    ...
    methods
        function obj = rosdevice(hostname, username, password, port)
            ...
            if nargin < 1
                ...
                assert(~isempty(hostname), message('robotics:robotslros:rosdevice:InvalidDeviceAddress'));
            else
            ...
            if nargin < 2
                ...
                assert(~isempty(username), message('robotics:robotslros:rosdevice:InvalidUsername'));
            else
            ...
            if nargin < 3
                ...
                assert(~isempty(password), message('robotics:robotslros:rosdevice:InvalidPassword'));
            else
            ...
            obj.Diagnostic = robotics.slros.internal.diag.DeviceDiagnostics;
    ...
    methods
        function runCore(obj)
            ...
            if obj.isCoreRunning
                disp(message('robotics:robotslros:rosdevice:ROSCoreAlreadyRunning').getString);
                return;
            end
            ...
            elseif obj.Diagnostic.isCatkinWorkspaceValid(obj.CatkinWorkspace)
                ...
                error(message('robotics:robotslros:rosdevice:UnknownROSFolder', obj.ROSFolder, obj.CatkinWorkspace));
            end
            ...
            if ~isCoreRunning
                ...
                error(message('robotics:robotslros:rosdevice:ROSCoreDidNotStart', logOut));
            end
        end
        ...
        function stopCore(obj)
            ...
            try
                ...
                if exMsg.contains('Operation not permitted')
                    ...
                    error(message('robotics:robotslros:rosdevice:StopROSCoreNoPrivileges', obj.Username));
        ...
        function runNode(obj, modelName, rosMasterURI, nodeHost)
            ...
            if obj.isNodeRunning(modelName)
                disp(message('robotics:robotslros:rosdevice:NodeAlreadyRunning', modelName).getString);
                return;
            end
            
            if nargin < 3
                % Use default MasterURI
                rosMasterURI = robotics.slros.internal.sim.defaultSimMasterURI(obj.DeviceAddress);
            else
                % Parse user input. The function displays an error if
                % the URI is not valid.
                rosMasterURI = robotics.ros.internal.Net.canonicalizeURI(rosMasterURI);
            end
            ...
            if nargin < 4
                ...
                if ~robotics.ros.internal.Net.isValidHost(nodeHost)
                    error(message('robotics:ros:util:HostnameInvalid', nodeHost));
                end
            end
        ...
        function stopNode(obj, modelName)
            ...
            try
                ...
                if exMsg.contains('Operation not permitted')
                    ...
                    error(message('robotics:robotslros:rosdevice:StopROSNodeNoPrivileges', modelName, obj.Username));
                elseif exMsg.contains('no process found')
    ...
    methods (Access = ?robotics.slros.internal.InternalAccess)
        function runNodeInternal(obj, modelName, rosMasterURI, nodeHost, cmdArgs)
            ...
            if ~obj.Diagnostic.doesFileExist(nodeExecutable)
                error(message('robotics:robotslros:rosdevice:NodeNotFound', nodeExecutable, modelName));
            end
            ...
            if n == numTries || logOut.contains('[ERROR]')
                error(message('robotics:robotslros:rosdevice:ROSNodeDidNotStart', nodeName, logFile, char(logOut)));
            end
        end
    end
