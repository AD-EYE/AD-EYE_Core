classdef LogfileDataLoader < handle
    ...
    % 
    %  Sample use:
    %   dlg = robotics.slros.internal.dlg.LogfileDataLoader.retrieveDialog(modelName);
    ...
    methods (Static)
        ...
        %% Dialog management
        function dlg = retrieveDialog(modelName)
        ...
        %   dlg - DAStudio.Dialog object
            ...
            dlg = findDDGByTag(strcat(robotics.slros.internal.dlg.LogfileDataLoader.DialogTagPrefix, modelName));
            if ~isempty(dlg)
                dlg = dlg(1);
                show(dlg)
            else
                obj = robotics.slros.internal.dlg.LogfileDataLoader(modelName);
                dlg = openDialog(obj);
            end
        end
    ...
    methods
        ...
        function obj = LogfileDataLoader(modelName)
        ...
        %               model's workspace
            ...
            [fname, ~, startOff, dur] = ...
                robotics.slros.internal.block.ReadDataBlockMask.getAllDataFromWS(obj.ModelName);
    ...
    methods (Hidden)
        ...
        function browseCallback(obj, dlg)
            filterSpec = '*.bag';
            titleTxt = getString(message('robotics:robotslros:readlog:BrowseTitle'));
            [filename, pathname] = ...
                uigetfile(filterSpec, titleTxt, obj.Filename);
            if ~isequal(filename, 0)
                filepath = [pathname, filename];
                obj.Filename = strtrim(filepath);
                refresh(dlg)
            end
        end
        ...
        %dlgApplyClose Callback for when the 'OK' button is pressed, but before the dialog closes
            ...
            try
                ...
                if isempty(obj.StartOffset)
                    ...
                    if isnan(startOff)
                        error(getString(message('robotics:robotslros:readlog:InvalidStartOffset')))
                    end
                end
                ...
                if isempty(obj.Duration)
                    ...
                    if isnan(dur) || dur < 0
                        error(getString(message('robotics:robotslros:readlog:InvalidDuration')))
                    end
                end
                
                % Set values in workspace
                robotics.slros.internal.block.ReadDataBlockMask.setAllDataInWS(...
        ...
        %getDialogSchema Determine the dialog window layout
            ...
            desc.WordWrap = true;
            desc.Name = getString(message('robotics:robotslros:readlog:LogfileDescText'));
            ...
            % Filename
            filename.Name = getString(message('robotics:robotslros:readlog:LogfilePathPrompt'));
            filename.Type = 'edit';
            ...
            % Browse button
            browse.Name = getString(message('robotics:robotslros:readlog:BrowsePrompt'));
            ...
            % Start time offset
            startoffset.Name = getString(message('robotics:robotslros:readlog:StartOffsetPrompt'));
            ...
            % Duration
            duration.Name = getString(message('robotics:robotslros:readlog:DurationPrompt'));
            duration.Type = 'edit';
            ...
            % Grouping
            groupBoxParams.Name       = getString(message('robotics:robotslros:readlog:LogfileOptionsTitle'));
            ...
            % Main dialog
            dlgstruct.DialogTitle = getString(message('robotics:robotslros:readlog:LoadLogfileTitle'));
