classdef ImageTopicTable < robotics.slros.internal.TopicTable 
    ...
    properties (Constant)
        ...
        NoTopicsAvailableMsgId = ...
            'robotics:robotslros:topicselector:NoImageTopicsAvailable'
    end
    ...
    methods
        ...
        function obj = ImageTopicTable(varargin)
            ...
            %   the robotics.ros.Node object, NODE. The table will stop
        ...
        function [status, enc, imgSize] = extractData(obj,msg)
            ...
            status = message('robotics:robotslros:topicselector:Valid').string;
            if isprop(msg,'Format') % CompressedImage
                format = msg.Format;
                parts = strsplit(format, ';');
                if numel(parts) ~= 2
                    status = message('robotics:ros:image:FormatNoSemicolon', ...
                            format).string;
                    return;
                end
                enc = lower(parts{1});
                if ~ismember(enc, {'rgb8', 'rgba8', 'bgr8', 'bgra8','mono8'})
                    status = message('robotics:ros:image:InvalidEncoding').string;
                    return
                end
                try
                    img = msg.readImage;
        ...
        function out = isSupportedEncoding(~, enc)
            ...
            out = robotics.slros.internal.block.ReadImage.ImageEncodingSet.isAllowedValue(enc);
        end
