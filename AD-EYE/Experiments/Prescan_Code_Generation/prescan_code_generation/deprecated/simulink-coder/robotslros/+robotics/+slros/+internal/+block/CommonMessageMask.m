classdef (Abstract) CommonMessageMask < robotics.slros.internal.block.CommonMask
...
%   Copyright 2015-2018 The MathWorks, Inc.
    ...
    methods
        ...
        function constantBlkInitFcn(obj, constantBlk)                     %#ok<INUSL>
            ...
            robotics.slros.internal.bus.Util.createBusIfNeeded(rosMsgType, bdroot(constantBlk));
        end

        function sysobjInitFcn(obj, sysobjBlock) %#ok<INUSL>
            rosMsgType = get_param(sysobjBlock, 'ROSMessageType');
            expectedBusName = robotics.slros.internal.bus.Util.rosMsgTypeToBusName(rosMsgType, bdroot(sysobjBlock));
    ...
    methods
        ...
        function topicEdit(obj, block)
            ...
            if ~robotics.ros.internal.Namespace.isValidGraphName(newValue)
                ...
                error(message('robotics:robotslros:blockmask:InvalidTopicName', newValue));
            end

            rosMsgType = get_param(block, 'messageType');
            robotics.slros.internal.bus.Util.createBusIfNeeded(rosMsgType, bdroot(block));
        ...
        function topicSelect(~, block)
            try
                msgDlg = robotics.slros.internal.dlg.TopicSelector();
        ...
        function messageTypeSelect(~, block)
            currentMsgType = get_param(block, 'messageType');

            msgDlg = robotics.slros.internal.dlg.MessageTypeSelector();
