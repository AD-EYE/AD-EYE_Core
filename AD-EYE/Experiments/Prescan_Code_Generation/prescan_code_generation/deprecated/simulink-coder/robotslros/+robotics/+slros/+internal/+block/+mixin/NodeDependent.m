classdef (Abstract) NodeDependent < handle
    ...
    properties (Abstract, Constant, Access=?robotics.slros.internal.block.mixin.NodeDependent)
    ...
    methods (Access = {?robotics.slros.internal.block.mixin.NodeDependent, ...
       ...
       function setupNodeDependent(obj)
            ...
            if coder.target('MATLAB')
                ...
                modelHasPriorState = robotics.slros.internal.sim.ModelStateManager.hasState(mdlName);
                
                try 
                    modelState = robotics.slros.internal.sim.ModelStateManager.getState(mdlName, 'create');
                    ...
                    if ~modelHasPriorState || ~modelState.nodeHasReferrers()
                        robotics.slros.internal.sim.ModelStateManager.clearState(mdlName);
                    end
            ...
            elseif coder.target('RtwForRapid')
                ...
                coder.internal.errorIf(true, 'robotics:robotslros:nodedependent:RapidAccelNotSupported', ...
       ...
       function releaseNodeDependent(obj)
           ...
           if coder.target('MATLAB')
               st = robotics.slros.internal.sim.ModelStateManager.getState(mdlName);
               st.decrNodeRefCount();
