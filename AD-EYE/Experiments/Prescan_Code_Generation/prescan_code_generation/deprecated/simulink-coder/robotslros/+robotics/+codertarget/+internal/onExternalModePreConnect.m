...
diagStage.reportInfo(message('robotics:robotslros:extmode:ExternalModeConnect', ...
...
% in robotics.codertarget.internal.DeploymentHooks.loadCommand
target = rosdevice;
...
if string(extModeAddress) ~= string(target.DeviceAddress)
    diagStage.reportWarning(message('robotics:robotslros:extmode:ExternalModeAddressMismatch', ...
...
if ~isRunning
    ...
    error(message('robotics:robotslros:extmode:ExternalModeDidNotStart', logFile, logContents));
end
