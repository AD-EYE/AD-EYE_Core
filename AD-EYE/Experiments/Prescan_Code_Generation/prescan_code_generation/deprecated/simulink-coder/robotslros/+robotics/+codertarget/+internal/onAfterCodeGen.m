...
%   Copyright 2014-2019 The MathWorks, Inc.
    ...
    lIsNewCode = robotics.codertarget.internal.hasModelChanged;
    if ~lIsNewCode && exist(archive, 'file')
        disp(message('robotics:robotslros:deploy:ArchiveUpToDate', modelName, archive).getString);
        return;
    end
    ...
    if ~robotics.codertarget.internal.Util.isTopLevelModel(buildInfo)
    ...
    sharedutilsdir = robotics.codertarget.internal.Util.sharedUtilsDir(buildInfo, true);
...
function writeCMakeListsTxt(buildInfo, cmakeVersion, packageName, packageDep, systemDep, msgFiles, msgDep)
    ...
    isTopLevelModel = robotics.codertarget.internal.Util.isTopLevelModel(buildInfo);

    % Find names for all referenced models.
    modelRefNames = robotics.codertarget.internal.Util.uniqueModelRefNames(buildInfo);
    ...
    for i = 1:numel(modelRefNames)
        packageDirName = robotics.codertarget.internal.Util.modelNameToValidPackageName(modelRefNames{i});
    ...
    % model references.
    sharedcpp = findLinkObject(buildInfo,'rtwshared.lib');
    ...
    for k = 1:numel(customLinkObjects)
        % Ignore rtwshared.lib as a custom link object. I will compile the
        % shared utility sources as part of the ROS build project.
        if strcmp(customLinkObjects.Name, 'rtwshared.lib')
            continue;
        end
    ...
    if isTopLevelModel
        for i = 1:numel(modelRefNames)
            actualLibName = robotics.codertarget.internal.Util.modelNameToValidPackageName(modelRefNames{i});
