classdef (Abstract) TopicTable < robotics.ros.internal.mixin.Unsaveable ...
    ...
    methods
        function obj = TopicTable(node, infoChangeFcn, timeout, allowEmptyTable)
            ...
            [allTopicNames, allMessageTypes] = ...
                robotics.ros.internal.NetworkIntrospection.getPublishedTopicNamesTypes(node.MasterURI);
            ...
            obj.TopicInfo.Status = repmat( ...
                message('robotics:robotslros:topicselector:WaitingForMessage').string , ...
                numTopics, 1);
            ...
            if numTopics > 0
                ...
                for k = 1:numTopics
                    obj.Subscribers{k} = ...
                        robotics.ros.internal.FastSubscriber(node, ...
        ...
        function killSubscribers(obj)
            for row = 1:numel(obj.Subscribers)
                if isvalid(obj.Subscribers{row})
                    obj.updateRow(row, ...
                        message('robotics:robotslros:topicselector:NoMessageReceived').string);
                    obj.Subscribers{row}.delete;
                end
            end
        end
