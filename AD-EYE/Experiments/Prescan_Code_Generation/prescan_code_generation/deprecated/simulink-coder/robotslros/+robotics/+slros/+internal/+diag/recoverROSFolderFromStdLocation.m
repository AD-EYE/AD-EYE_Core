...
for i = 1:length(rosFolders)
    ...
    if i == defaultROSFolderIndex
        ...
        msgArray{i} = message('robotics:robotslros:devicediag:ROSFolderRecoverySuccess', upper(distName), rosFolder).getString;
    else
        msgArray{i} = message('robotics:robotslros:devicediag:ROSFolderRecoverySuccessNondefault', upper(distName), rosFolder).getString;
    end
end
