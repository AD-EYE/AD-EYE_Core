...
validateattributes(modelinfo, ...
    {'robotics.slros.internal.cgen.ROSModelInfo'}, {'scalar'});
validateattributes(destDir, {'char'}, {});

% Define info about generated code
cgenConsts = robotics.slros.internal.cgen.Constants;
initinfo.Includes = {...
...
robotics.slros.internal.cgen.insertHeaderGuards(headerFile, cgeninfo.HeaderFileName);
...
for i=1:numel(varInfo)
    if ~isempty(varInfo(i).Comment)
        commentStr = robotics.slros.internal.cgen.Util.cleanupUnicodeAndWhitespace(varInfo(i).Comment);
...
for i=1:numel(modelinfo.PublisherList)
   ...
   varInfo(end+1)= struct(...
       'Comment', pub.Comment, ...
       'Declaration', sprintf('SimulinkPublisher<%s, %s> %s', ...
                      pub.CppRosType, pub.SlBusName, pub.Label),...
       'Value', '');   
end   
