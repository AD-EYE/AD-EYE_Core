...
rosMaster = robotics.slros.internal.sim.ROSMaster;
masterURI = rosMaster.MasterURI;

if strfind('http://localhost:11311', masterURI)
    ...
    try
        ...
        hostIP = robotics.ros.internal.Net.getAssociatedInterfaceAddress(targetAddress);
    catch
        % If this fails, try to determine the IP address by pinging
        % the ROS device through each interface
        intfInfo = robotics.ros.internal.Net.getAllIPv4Addresses;
        hostIP = '';
        ...
        if isempty(hostIP)
            ...
            warning(message('robotics:robotslros:deploy:UnabletoDetermineROSMasterIP',...
