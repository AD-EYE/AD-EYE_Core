classdef  ServiceCallListStream < robotics.slros.internal.sim.ServiceCallStream
    ...
    properties (Transient, SetAccess = ?matlab.unittest.TestCase)        
        ...
        LastCallRequest = robotics.ros.Message.empty(0,1)
    end
    ...
    methods       
        ...
        function [resp, errorCode] = callService(obj, req)
            ...
            if isempty(obj.ResponseList)
                error("robotics:slros:internal:ResponseListEmpty", "Cannot retrieve service response, because the ResponseList cell array is empty.");
            end
