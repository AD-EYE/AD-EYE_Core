classdef ReadData < matlab.System & ...
    ...
    %
    %   H = robotics.slros.internal.block.ReadData creates a system
    ...
    %   block. In order to access the rosbag playback functionality from 
    %   MATLAB, see ROSBAG.
    ...
    properties (Access = protected, Transient)
        ...
        DataSelection = robotics.ros.BagSelection.empty;
    ...
    methods
        ...
        %set.SampleTime Validate sample time specified by user
            ...
            coder.internal.errorIf(numel(sampleTime) > 2, ...
                'robotics:robotslcore:sampletime:InvalidSampleTimeNeedScalar');
    ...
    methods(Access = protected)
        ...
        %   data object will be stored.
            ...
            dataobjVal = robotics.slros.internal.block.ReadDataBlockMask.getDataFromWS(...
                robotics.slros.internal.block.ReadDataBlockMask.DataObjectVarName, ...
                dataobj, obj.ModelName);
            
            if isa(dataobjVal, 'robotics.ros.BagSelection')
                dataobj = dataobjVal;
            end
        ...
        %   playing back the entire data file.
            ...
            offsetVal = robotics.slros.internal.block.ReadDataBlockMask.getDataFromWS(...
                robotics.slros.internal.block.ReadDataBlockMask.PlaybackOffsetVarName, ...
                offset, obj.ModelName);
            durationVal = robotics.slros.internal.block.ReadDataBlockMask.getDataFromWS(...
        ...
        %   the playback window.
            ...
            if isempty(obj.Topic)
                error(message('robotics:robotslros:readlog:NoTopicName', mfilename('class')))
            end
            if ~contains(obj.Topic, obj.DataObject.AvailableTopics.Row)
                error(message('robotics:robotslros:readlog:InvalidTopicName', obj.Topic, mfilename('class')))
            end
        ...
        %   Will error if not in a Simulink model.
            ...
            obj.Converter = robotics.slros.internal.sim.ROSMsgToBusStructConverter(...
                obj.MsgType, obj.ModelName);
            obj.LastConvertedMsg = obj.Converter.convert(rosmessage(obj.MsgType));
            ...
            % Still messages left to output
                ...
                if nextMsgTimestamp > currentLogTime
                    ...
                    if obj.IsControllableSampleTime
                        ...
                        stepsToNextMsg = ...
                            ceil(timeToNextMsg/obj.SampleTime);
                        stepsToNextMsg = clipNumTicks(stepsToNextMsg);
                        setNumTicksUntilNextHit(obj, stepsToNextMsg);
                    end
                ...
                % Time to output new message
                    ...
                    msgRaw = msgRaw{1};
                    msg = convert(obj.Converter, msgRaw);
                    isNew = true;
        ...
        %getOutputDataTypeImpl Indicates datatype of all output signals
            ...
            if ~isempty(obj.ModelName)
                if ~isempty(obj.MsgType)
                    robotics.slros.internal.bus.Util.createBusIfNeeded(...
                        obj.MsgType, obj.ModelName);
                    [~, slBusName] = ...
                        robotics.slros.internal.bus.Util.rosMsgTypeToDataTypeStr(...
                        obj.MsgType, obj.ModelName);
                else
        ...
        %   applicable.
            ...
            if isequal(obj.SampleTime, -2)          % Variable
                ...
                refreshDataSelection(obj)
                tGap = robotics.slros.internal.block.ReadDataBlockMask.getMinNonzeroMsgGap(obj.DataSelection);
