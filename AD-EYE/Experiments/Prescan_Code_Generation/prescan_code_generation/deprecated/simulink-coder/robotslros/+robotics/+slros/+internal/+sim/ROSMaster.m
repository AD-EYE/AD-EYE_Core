classdef ROSMaster
    ...
    properties
        TimeoutInSeconds = robotics.ros.internal.Settings.DefaultMasterConnectionTimeout
    end
    ...
    methods
        ...
        function obj = ROSMaster(profile)
            if exist('profile', 'var')
                ...
                netAddrStore = robotics.slros.internal.sim.NetworkAddrStore;
            ...
            if obj.Profile.MasterUseDefault
                ...
                obj.MasterURI = robotics.ros.internal.Net.generateMasterURI(true);
            else
                ...
                obj.MasterURI = robotics.ros.internal.Net.generateMasterURI(...
        ...
        function isReachable = isReachable(obj)
            isReachable  = robotics.ros.internal.NetworkIntrospection.isMasterReachable(...
        ...
        function verifyReachable(obj)
            if ~obj.isReachable()
                ex = MSLException([], message('robotics:robotslros:ros:ROSMasterNotReachable', obj.MasterURI));
                throw(ex);
            end
        end
        ...
        function [topics, msgtypes] = getTopicNamesTypes(obj)
            ...
            [topics, msgtypes] = ...
                robotics.ros.internal.NetworkIntrospection.getPublishedTopicNamesTypes(obj.MasterURI);
        end
        ...
        function [serviceNames, serviceTypes] = getServiceNamesTypes(obj)
            ...
            serviceAndNodes = robotics.ros.internal.NetworkIntrospection.getServiceList(obj.MasterURI);
            ...
            for i = 1:length(serviceNames)
                ...
                serviceTypes{i} = robotics.ros.internal.NetworkIntrospection.getServiceType(svcName, obj.MasterURI);
            end
        end
        ...
        function node = createNode(obj, name)
            ...
            canonicalName = robotics.ros.internal.Namespace.canonicalizeName(name);
            if obj.Profile.NodeUseDefault
                node = robotics.ros.Node(canonicalName, obj.MasterURI);
            else
                node = robotics.ros.Node(canonicalName, obj.MasterURI, ...
