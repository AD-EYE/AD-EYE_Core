...
class GridMapCreator
{
  private:
    ...
    {
        // Determine the boundaries of the map based on the maximum and minimum values for x and y as saved in the
        // Vector Map
        float lowest_x;
        float lowest_y;
        float maplength_x;
    ...
    void addPrescanObjects(ros::NodeHandle& p_nh)
    {
        ...
        if (file.empty())
        {
            ROS_ERROR_STREAM("No .csv files describing the UserLibrary Element has been found for resolution "
                             << map_resolution_);
        }
        else
        {
        ...
        // pex file data is used to built the staticObjects layer (stuff like buildings, nature, traffic lights), the
        // value given to the cells is the height of the static object

        // building objects are always considered to be rectangles, and as such, a rectangle is created and the
        // PolygonIterator is used. The number 0.01745 is a conversion from degrees to radians
        for (int i = 0; i < (int)pexObjects.buildingObjects.size(); i++)
        {
            grid_map::Polygon polygon =
                createRectangle(pexObjects.buildingObjects.at(i).posX, pexObjects.buildingObjects.at(i).posY,
                                pexObjects.buildingObjects.at(i).sizeX, pexObjects.buildingObjects.at(i).sizeY,
        ...
        // Nature is created using the circleIterator, as trees and bushes do not really have a definite shape, a circle
        // describes their shape the best
        ...
        for (int i = 0; i < (int)pexObjects.roadSideParkingObjects.size(); i++)
        {
            grid_map::Polygon polygon = createRectangle(
                pexObjects.roadSideParkingObjects.at(i).posX, pexObjects.roadSideParkingObjects.at(i).posY,
                pexObjects.roadSideParkingObjects.at(i).sizeX, pexObjects.roadSideParkingObjects.at(i).sizeY,
            ...
            for (grid_map::PolygonIterator it(map_, polygon); !it.isPastEnd(); ++it)
            {
                map_.at("RoadSideParking", *it) = pexObjects.roadSideParkingObjects.at(i).roadSideParkingValue +
                                                  i;  // value is different for each area o avoid clustering later
            }
        }
        ...
        {
            grid_map::Polygon polygon =
                createRectangle(pexObjects.restAreaObjects.at(i).posX, pexObjects.restAreaObjects.at(i).posY,
                                pexObjects.restAreaObjects.at(i).sizeX, pexObjects.restAreaObjects.at(i).sizeY,
        ...
        {
            grid_map::Polygon polygon =
                createRectangle(pexObjects.staticCarsObjects.at(i).posX, pexObjects.staticCarsObjects.at(i).posY,
                                pexObjects.staticCarsObjects.at(i).sizeX, pexObjects.staticCarsObjects.at(i).sizeY,
    ...
    void addVectorMapObjects(VectorMap vector_map)
    {
        ...
        float lanewidth;
        float laneID;
        for (int i = 0; i < static_cast<int>(vector_map.lanes_startpoint_.size()); i++)
        {  // For every lane
            if (!nh_.ok())
            {
                break;
            ...
            point1.x() = vector_map.points_x_.at(vector_map.nodes_pid_.at(vector_map.lanes_startpoint_.at(i) - 1) -
                                                 1);  // Get the first
            point1.y() = vector_map.points_y_.at(vector_map.nodes_pid_.at(vector_map.lanes_startpoint_.at(i) - 1) - 1);
            point2.x() = vector_map.points_x_.at(vector_map.nodes_pid_.at(vector_map.lanes_endpoint_.at(i) - 1) -
                                                 1);  // and the second point
    ...
    void dynamicActorsUpdateGroundTruth()
    {
        ...
        for (int i = 0; i < (int)N_actors; i++)
        {
            ...
            if (dynamic_objects_ground_truth_initialized_)
            {
                ...
                if (x_other - x_ego_old_ < submap_dimensions_ && x_other - x_ego_old_ > -submap_dimensions_ &&
                    y_other - y_ego_old_ < submap_dimensions_ && y_other - y_ego_old_ > -submap_dimensions_)
                {
                    grid_map::Polygon otherCarOld =
                        createRectangle(x_other, y_other, length_other_, width_other_, yaw_other);
            ...
            if (x_other - x_ego_ < submap_dimensions_ && x_other - x_ego_ > -submap_dimensions_ &&
                y_other - y_ego_ < submap_dimensions_ && y_other - y_ego_ > -submap_dimensions_)
            {
    ...
    /*!
     * \brief Dynamic Objects Ground TruthCallback : Called when the position of the dynamic objects sent by Simulink
     * has changed.
     * \param msg A smart pointer to the message from the topic.
     ...
     * \param msg A smart pointer to the message from the topic.
     * \details When the car is moving, this function will delete old polygons and add new polygons corresponding to the
   ...
   void sensorSectorsCallback(const jsk_recognition_msgs::PolygonArray::ConstPtr& msg)
    {
        // On PX2, the layer sensorSector was not useful ans used a lot of CPU, that why it is commented here. 
	/*!sensor_sectors_ = *msg;
    ...
    static grid_map::Polygon createRectangle(float x, float y, float length, float width, float angle)
    {
        ...
        polygon.setFrameId("SSMP_map");
        polygon.addVertex(
            Position(x + length * cos(angle) - width * sin(angle), y + width * cos(angle) + length * sin(angle)));
        polygon.addVertex(
            Position(x + length * cos(angle) + width * sin(angle), y - width * cos(angle) + length * sin(angle)));
        polygon.addVertex(
            Position(x - length * cos(angle) + width * sin(angle), y - width * cos(angle) - length * sin(angle)));
                              ...
                              std::pow(occmap_height_, 2)))
    {
        ...
        if (use_ground_truth_dynamic_objects_)
            sub_dynamic_objects_ground_truth_ = nh.subscribe<geometry_msgs::PoseArray>(
                "/pose_otherCar", 1, &GridMapCreator::dynamicObjectsGroundTruthCallback, this);
        else
            sub_dynamic_objects_ = nh.subscribe<jsk_recognition_msgs::PolygonArray>(
                "/safetyChannelPerception/safetyChannelPerception/detection/polygons", 1,
                &GridMapCreator::dynamicObjectsCallback, this);

        sub_sensor_fov_ = nh.subscribe<jsk_recognition_msgs::PolygonArray>(
            "/sensor_fov", 1, &GridMapCreator::sensorSectorsCallback, this);

        // these three variables determine the performance of gridmap, the code will warn you whenever the performance
