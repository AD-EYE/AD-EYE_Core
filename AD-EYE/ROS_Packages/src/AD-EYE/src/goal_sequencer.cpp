...
class GoalSequencer
{
  private:
    ...
    void storeGoalCoordinatesCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
    {
        ...
        if (getDistance(goal_coordinates_.back().pose.position.x, msg->pose.position.x,
                        goal_coordinates_.back().pose.position.y, msg->pose.position.y) > DISTANCE_TOLERANCE_)
        {
            goal_coordinates_.push(*msg);

            // Print the new goal positions
            ROS_INFO("The next goal has been received. Position:- x = %lf, y = %lf, z = %lf",
                     goal_coordinates_.back().pose.position.x, goal_coordinates_.back().pose.position.y,
                     goal_coordinates_.back().pose.position.z);
        }
    }
  ...
  public:
    ...
    GoalSequencer(ros::NodeHandle& nh) : nh_(nh), rate_(20)
    {
        ...
        sub_autoware_global_plan_ = nh_.subscribe<autoware_msgs::LaneArray>(
            "/lane_waypoints_array", 1, &GoalSequencer::autowareGlobalPlanCallback, this);
