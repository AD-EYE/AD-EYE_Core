...
class ExperimentARecorder:
    ...
    def lidarObjectCallback(self, data):
        for object in data.objects:
            try:
                ...
                if((object_pose.pose.position.x - PEDESTRIAN_END_POSITION[0])**2 + (object_pose.pose.position.y - PEDESTRIAN_END_POSITION[1])<25):
    ...
    def checkExperimentEnd(self):
        ...
        if len(self.ego_speeds)>1 and rospy.get_rostime().secs > 5 : # in order to have access to the previous speed and to make sure the experiment is not stopped at the begining due to noise
            ...
            if len(self.distances_ego_pedestrian)>2 :
                if self.distances_ego_pedestrian[len(self.distances_ego_pedestrian)-3]<self.distances_ego_pedestrian[len(self.distances_ego_pedestrian)-2] : # if the distance is increasing, the pedestrian has been passed,
                    ...
                    if self.collision == False : # if the collision didn't occurred
                        ...
                        if (self.ego_speeds[len(self.ego_speeds)-1]==0.0) and (self.ego_speeds[len(self.ego_speeds)-2]!=0.0): # if the car just stoped (to test it only once)
