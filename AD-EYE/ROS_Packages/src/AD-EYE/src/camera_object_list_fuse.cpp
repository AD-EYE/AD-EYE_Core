...
public:
     ...
     * \details Initialize the node and its components such as publishers and subscribers. It call msg1Callback and msg2Callback
     */
    CameraObjectListFuse(ros::NodeHandle &nh, std::string input_topic_1, std::string input_topic_2, std::string output_topic) : nh_(nh)
    {
        // Initialize the publishers and subscribers
        sub1_ = nh_.subscribe<autoware_msgs::DetectedObjectArray>(input_topic_1, 1, &CameraObjectListFuse::msg1Callback, this);
        sub2_ = nh_.subscribe<autoware_msgs::DetectedObjectArray>(input_topic_2, 1, &CameraObjectListFuse::msg2Callback, this);
        ...
        for (size_t i = 0; i < msg2_.objects.size(); i++) {
            if (msg2_.objects.at(i).label != "unknown") {
                for (size_t j = 0; j < msg3_.objects.size(); j++) {
                    if (msg3_.objects.at(j).user_defined_info.size() > 0 && msg2_.objects.at(i).user_defined_info.size() > 0) {
