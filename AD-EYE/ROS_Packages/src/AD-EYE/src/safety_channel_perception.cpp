...
class SafetyChannelPerception
{
  private:
    ...
    * \details Gets the Point Cloud clusters and transform each point of their convex hull footprints into the frame
    ...
    */
    void clusterCallback(const autoware_msgs::CloudClusterArray::ConstPtr& msg)
    {
        ...
        for (auto cluster : msg->clusters)
        {
            ...
            {
                if (pt.z == z)
                {  // to get only the footprint, autoware's cluster hulls are composed of two layers: the footprint and
                   // the same shape but above the cluster
  ...
  public:
    ...
    */
    SafetyChannelPerception(ros::NodeHandle& nh, tf::TransformListener& listener)
      : nh_(nh), rate_(10), tf_listener_(listener)
    {
        ...
        pub_polygons_ =
            nh.advertise<jsk_recognition_msgs::PolygonArray>("safetyChannelPerception/detection/polygons", 1, true);
    }
