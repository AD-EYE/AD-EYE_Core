  ...
  private:
    ...
    void storeGoalCoordinatesCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
    {
        if (!received_vector_mapper_)
        {
            ROS_WARN(
                ...
                "typing for example:");
            ROS_WARN("roslaunch adeye my_map.launch world_name:=W01_Base_Map");
        }
        ...
        {
            ROS_ERROR("The received goal is not valid");
        }
    }
    ...
    */
    bool isPointCloseToVM(const double& distance)
    {
        ...
        {
            ROS_ERROR_STREAM("Recieved goal is not within the vector map: " << distance);
            return false;
        }
    }
    ...
    */
    bool isPointAlignedWithVM(const int& vmap_orientation, const double& yaw_angle)
    {
        ...
        {
            ROS_ERROR_STREAM("Recieved orientation difference from the vector map is too large: " << yaw_angle << " vs. " <<  vmap_orientation);
            return false;
        }
    }
    ...
    */
    ...
    bool areGoalsDifferentEnough(const double& x, const double& y, const double& yaw_angle)
    {
        ...
        {
            ROS_ERROR("Distance or orientation between current goal and previous goal is not large enough");
            return false;
        }
    }
