...
ObstaclesInCriticalAreaChecker::ObstaclesInCriticalAreaChecker(int increment_value, int decrement_value,
  ...
  : SafetyFaultMonitor(increment_value, decrement_value, high_threshold, low_threshold)
{
    ...
    sub_gridmap_ = nh_.subscribe<grid_map_msgs::GridMap>("/safety_planner_gridmap", 1,
                                                         &ObstaclesInCriticalAreaChecker::gridmapCallback, this);
    sub_autoware_trajectory_ = nh_.subscribe<autoware_msgs::Lane>(
        "/final_waypoints", 1, &ObstaclesInCriticalAreaChecker::autowareTrajectoryCallback, this);
    sub_current_velocity_ =
        nh_.subscribe("/current_velocity", 1, &ObstaclesInCriticalAreaChecker::currentVelocityCallback, this);
    pub_critical_area_ =
        nh_.advertise<visualization_msgs::Marker>("/critical_area", 1, true);  // Used for critical area visualization
}
...
void ObstaclesInCriticalAreaChecker::updateCriticalAreaShape()
{  // Remove critical area vertices
    ...
    for (int k = 0; k < autowareTrajectory_.waypoints.size() - 1; k++)
    {
        ...
        double distance_between_two_waypoints =
            getDistance(autowareTrajectory_.waypoints.at(k + 1).pose.pose.position.x,
                        autowareTrajectory_.waypoints.at(k).pose.pose.position.x,
                        autowareTrajectory_.waypoints.at(k + 1).pose.pose.position.y,
                        autowareTrajectory_.waypoints.at(k).pose.pose.position.y);
