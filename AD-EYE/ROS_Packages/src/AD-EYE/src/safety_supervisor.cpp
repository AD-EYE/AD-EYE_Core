...
class SafetySupervisor
{
  private:
    ...
    enum SENSOR_TYPE_
    {
        ...
        camera2,
        cameratl
    };  // numbers of sensors have to fit with those defined in sensor monitor
    ...
    double getDistanceToLane(const std::vector<PlannerHNS::WayPoint>& trajectory)
    {
        ...
        double distance = double(fabs((p2.pos.y - p1.pos.y) * p0.pos.x - (p2.pos.x - p1.pos.x) * p0.pos.y +
     ...
     */
    std::vector<int> getTwoClosestIndices(const std::vector<PlannerHNS::WayPoint>& trajectory,
