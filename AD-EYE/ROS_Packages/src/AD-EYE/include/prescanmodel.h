...
struct PrescanModel
{
    ...
    */
     ...
     */
    void load_actor(boost::property_tree::ptree::value_type& local_ptree, int u_id, int type_id,
                    int road_side_parking_value, int rest_area_value)
    {
        ...
        BOOST_FOREACH (ptree::value_type& val_experiment, pt.get_child("Experiment"))
        {
            ...
            if (val_experiment.first == "Actors")
            {
                ...
                BOOST_FOREACH (ptree::value_type& val_actors, pt_actors)
                {
                    ...
                    {
                        // std::cout << "Values: " << val_actor_attr.first.data() << ":"
                        // <<val_actor_attr.second.data()<< std::endl;
                        ...
                        // std::cout << "After Values: " << val_actor_attr.first.data() << ":" <<u_id<<":" <<obj_id<<
                        // std::endl;
                    }
                        ...
                        BOOST_FOREACH (ptree::value_type& val_underlays, val_experiment.second.get_child("Underlays"))
                        {
                            ...
                            BOOST_FOREACH (ptree::value_type& val_underlay_attr, pt_Underlay_Attr)
                            {
                                ...
                                if (val_underlay_attr.first == "Description")
                                {
                                    ...
                                    // if(val_underlay_attr.second.data().size()>=SAFE_STRING_POS &&
                                    // val_underlay_attr.second.data().substr(0,SAFE_STRING_POS) == "SAFE")
                                    // {
                                    //     safety_area_value =
                                    //     strtol(val_underlay_attr.second.data().substr(SAFE_STRING_POS).c_str(),NULL,DECIMAL_BASE);
                                    // }
                                    // checking the condition if the attritbute "Description" has the value
                                    // "roadSideParking" in it
                                    if (val_underlay_attr.second.data().size() >= ROAD_SIDE_PARKING_STRING_POS &&
                                        val_underlay_attr.second.data().substr(0, ROAD_SIDE_PARKING_STRING_POS) ==
                                            "roadSideParking")
                                    {
                                        road_side_parking_value = ROAD_SIDE_PARKING_VALUE;
                                    }
                                    // checking the condition if the attribute "Description" has the value "restArea" in
                                    // it
                                    if (val_underlay_attr.second.data().size() >= REST_AREA_STRING_POS &&
                                        val_underlay_attr.second.data().substr(0, REST_AREA_STRING_POS) == "restArea")
                                    {
