...
namespace kcan
{
    ...
    E2EProtector::E2EProtector(const string &sg_name, uint cntr_init) : sgi_(DBCReader::getSignalGroupInfo(sg_name))
    {
...
*/
        //Std_ReturnType st = E2E_P01Protect(&profile1a, &state->protect, data.data() + sg_start_byte_);
        //Std_ReturnType st = E2E_P01Protect(&profile1a, &state->protect, data.data() + 0);
//        data = d;
        //return st;
    }
    ...
    E2EResult E2EProtector::applyProfile05(const SignalValues& sv, const E2E_P05ConfigType* config)
    {
        ...
        Std_ReturnType status = E2E_P05Protect(&profile5, &state->protect, signals.data(), signals.size());
        if (status != E2E_E_OK) {
            throw runtime_error("E2E Error");
        }
    ...
    void E2EProtector::packSignals(const SignalValues& sv, vector<uint8_t>& signals, uint counter_pos) {
        for (auto &el : sv) {
            ...
            bool is_ones_padding = false;  //(1UL << (si.length - 1) & sig_value) && si.type != SignalType::APP_SIGNED;
