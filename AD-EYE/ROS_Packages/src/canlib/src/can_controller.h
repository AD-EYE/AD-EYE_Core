    ...
    CANController(CANBus bus) {
        ...
        */
        ...
        if (type == "socketcan") {
            ...
            bool receiving_log_on = reader.GetBoolean(section, "receiving_log_on", false);
            bool sending_log_on = reader.GetBoolean(section, "sending_log_on", false);
            can_controller_ = std::unique_ptr<SOCKETCANController>(
  ...
  private:
    ...
    string busToIniSection(CANBus bus) {
        ...
        case CANBus::D:
            return "CAN_D";
        default:
            throw invalid_argument("Unknown bus!");
        }
    }
};
