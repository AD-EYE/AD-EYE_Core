 ...
 */
static INLINE uint16 computeCRCP05(const uint8* DataPtr, uint16 length, uint16 offset, uint16 dataId, uint16 start) {
    ...
    if (offset > 0) {
        ...
        crc = Crc_CalculateCRC16(&DataPtr[0], offset, start, TRUE);
        ...
        /* Compute CRC over bytes that are after CRC (if any) */
        crc = Crc_CalculateCRC16(&DataPtr[offset+2], lengthBytes, start, TRUE);
    }
...
Std_ReturnType E2E_P05Protect(const E2E_P05ConfigType* ConfigPtr, E2E_P05ProtectStateType* StatePtr, uint8* DataPtr, uint16 Length) {
    ...
    if (E2E_E_OK == returnValue) {
        ...
        crc = computeCRCP05(DataPtr, Length, offset, ConfigPtr->DataID, 0xFFFFu);
...
Std_ReturnType E2E_P05Check(const E2E_P05ConfigType* ConfigPtr, E2E_P05CheckStateType* StatePtr, const uint8* DataPtr, uint16 Length) {
    ...
    if (E2E_E_OK == returnValue) {
        if (NewDataAvailable == TRUE) {
            ...
            computedCRC = computeCRCP05(DataPtr, Length, offset, ConfigPtr->DataID, 0xFFFFu);
        }
